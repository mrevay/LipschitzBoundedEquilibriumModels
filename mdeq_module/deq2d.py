...
from mdeq_module.broyden import broyden, analyze_broyden
...
class DEQFunc2d(Function):
    ...
    def broyden_find_root(func, z1, u, eps, *args):
        ...
        def g(x): return DEQFunc2d.g(func, x, u, cutoffs, *args)
        result_info = broyden(
            g, z1_est, threshold=threshold, eps=eps, name="forward")
        ...
        if z1_est.get_device() == 0:
            if writer is not None:
                writer.add_scalar(
                    'forward/diff', result_info['diff'], train_step)
                writer.add_scalar(
                    'forward/nstep', result_info['nstep'], train_step)
                writer.add_scalar('forward/lowest_step',
                                  result_info['lowest_step'], train_step)
                writer.add_scalar(
                    'forward/final_trace', result_info['new_trace'][lowest_step], train_step)

        status = analyze_broyden(result_info, judge=True)
        if status:
            err = {"z1": z1}
            analyze_broyden(result_info, err=err, judge=False,
                            name="forward", save_err=False)
    ...
    def forward(ctx, func, z1, u, *args):
        ...
        with torch.no_grad():
            # args include pos_emb, threshold, train_step
...
class DEQModule2d(nn.Module):
    ...
    class Backward(Function):
        ...
        def backward(ctx, grad):
            ...
            cutoffs = [(elem.size(1) // factor, elem.size(2), elem.size(3))
                       for elem in u]
            args = ctx.args
            threshold, train_step, writer = args[-3:]
            ...
            result_info = broyden(
                g, dl_df_est, threshold=threshold, eps=eps, name="backward")
            ...
            if dl_df_est.get_device() == 0:
                if writer is not None:
                    writer.add_scalar(
                        'backward/diff', result_info['diff'], train_step)
                    writer.add_scalar('backward/nstep',
                                      result_info['nstep'], train_step)
                    writer.add_scalar('backward/lowest_step',
                                      result_info['lowest_step'], train_step)
                    writer.add_scalar(
                        'backward/final_trace', result_info['new_trace'][lowest_step], train_step)

            status = analyze_broyden(result_info, judge=True)
            if status:
                err = {"z1": z1}
                analyze_broyden(result_info, err=err, judge=False,
                                name="backward", save_err=False)
